...
</dialog><dialog id="update-dialog" class="about-dialog" aria-labelledby="update-dialog-title"><section class="about-dialog-card"><p class="eyebrow">Оновлення</p><h2 id="update-dialog-title">Перевірка оновлень</h2><p id="update-dialog-message" class="update-dialog-message" aria-live="polite">Перевіряємо наявність нової версії…</p><div class="about-dialog-actions"><button id="update-close" class="primary-button" type="button">Закрити</button></div></section></dialog><footer class="app-version"><span id="active-tab-name">Розпізнавання</span> | Обладнання та дані · by <a id="external-homeandriy" href="https://webbooks.com.ua">homeandriy</a> · v{{ $appVersion }}</footer></main><script>window.initialAiAgents=@json($agents);</script><script src="{{ asset('js/app.js') }}?v={{ filemtime(public_path('js/app.js')) }}"></script>@if ($setupRequired)
