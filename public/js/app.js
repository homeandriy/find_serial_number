...
const status = document.querySelector("#status"),
    ...
    localResult = document.querySelector("#local-result"),
    aiResult = document.querySelector("#ai-result"),
    agentSelect = document.querySelector("#agent-select"),
    aiButton = document.querySelector("#recognize-ai"),
    refreshButton = document.querySelector("#refresh-folder"),
    agentForms = document.querySelector("#agent-forms");
let agents = window.initialAiAgents || [],
...
const request = async (url, method = "GET", body) => {
    ...
    if (!response.ok)
        throw new Error(payload.message || "Не вдалося виконати запит.");
    return payload;
};
...
function renderAgentSelect() {
    ...
    agentSelect.innerHTML = agents.length
        ? '<option value="">Виберіть агента</option>' +
          agents
              .map(
                  (agent) =>
                      '<option value="' +
              ...
              .join("")
        : '<option value="">Спочатку додайте агента</option>';
    agentSelect.disabled = !agents.length;
    if (agents.some((agent) => agent.id === value)) agentSelect.value = value;
    updateAiState();
}
function updateAiState() {
    const ready = Boolean(selectedImageId && agentSelect.value);
    aiButton.disabled = !ready;
...
function agentForm(agent = {}) {
    return (
        ...
        escapeHtml(agent.name || "") +
        '" placeholder="Наприклад, OpenAI OCR"></label><label>Постачальник<select name="provider">' +
...
function renderForms() {
    agentForms.innerHTML = agents.map((agent) => agentForm(agent)).join("");
    bindForms();
}
function bindForms() {
    agentForms.querySelectorAll(".agent-form").forEach((form) => {
        form.addEventListener("submit", async (event) => {
            event.preventDefault();
            const message = form.querySelector(".agent-message"),
                id = form.dataset.agentId,
                data = Object.fromEntries(new FormData(form));
            message.textContent = "Збереження…";
            try {
                const payload = await request(
                    id ? "/ai-agents/" + id : "/ai-agents",
                    id ? "PUT" : "POST",
                    data,
                );
                if (id)
                    agents = agents.map((agent) =>
                        agent.id === id ? payload.agent : agent,
                    );
                else agents.push(payload.agent);
                renderForms();
                renderAgentSelect();
            } catch (error) {
            ...
            async () => {
                ...
                try {
                    ...
                    agents = agents.filter(
                        (agent) => agent.id !== form.dataset.agentId,
                    );
                    renderForms();
                    renderAgentSelect();
                } catch (error) {
                    form.querySelector(".agent-message").textContent =
                        error.message;
                }
            },
        );
        ...
        document
            ...
            .forEach((item) =>
                item.classList.toggle(
                    "is-active",
                    item.id === button.dataset.tab + "-tab",
                ),
            );
    }),
);
document.querySelector("#add-agent")?.addEventListener("click", () => {
    agentForms.insertAdjacentHTML("beforeend", agentForm());
    bindForms();
    agentForms.lastElementChild?.scrollIntoView({
        behavior: "smooth",
        block: "center",
    });
});
...
document.querySelectorAll(".image-card").forEach((card) =>
    card.addEventListener("click", async () => {
        ...
        try {
            const payload = await request(
                "/images/" + selectedImageId + "/recognize",
                "POST",
            );
...
aiButton.addEventListener("click", async () => {
    ...
    aiResult.value = "AI-розпізнавання…";
    status.textContent = "AI аналізує зображення…";
    try {
        const payload = await request(
            "/images/" + selectedImageId + "/recognize-ai/" + agentSelect.value,
            "POST",
        );
        aiResult.value = payload.text || "Текст не знайдено.";
        status.textContent = "AI-розпізнавання готове";
    } catch (error) {
        aiResult.value = error.message;
        status.textContent = "Помилка AI";
    } finally {
        updateAiState();
    }
});
...
const textContextMenu = document.createElement("button");
textContextMenu.type = "button";
textContextMenu.className = "text-context-menu";
textContextMenu.textContent = "Копіювати";
textContextMenu.hidden = true;
document.body.append(textContextMenu);
...
document.addEventListener("contextmenu", (event) => {
    ...
    textContextMenu.style.left = event.clientX + "px";
    textContextMenu.style.top = event.clientY + "px";
    contextSelection = selected;
    event.preventDefault();
});
textContextMenu.addEventListener("click", async () => {
    try {
        await navigator.clipboard.writeText(contextSelection);
        status.textContent = "Виділений текст скопійовано";
    } catch {
        status.textContent = "Не вдалося скопіювати виділений текст";
    } finally {
        textContextMenu.hidden = true;
    }
});
...
devicesTab.className = "tab-content";
devicesTab.innerHTML =
    '<section class="settings-panel"><h2>Обладнання</h2><form id="device-form" class="agent-form"><label>Виділений текст<textarea name="recognized_text" id="device-text" required></textarea></label><label>Модель<select name="device_model_id" id="device-model" required></select></label><label>Дата<input name="registered_at" type="date" value="' +
    new Date().toISOString().slice(0, 10) +
    '" required></label><button class="primary-button">Зберегти</button></form></section>';
document.querySelector(".app-shell").append(devicesTab);
deviceTabButton.addEventListener("click", () => {
    document
        .querySelectorAll(".tab-button")
        .forEach((x) => x.classList.toggle("is-active", x === deviceTabButton));
    document
        .querySelectorAll(".tab-content")
        .forEach((x) => x.classList.toggle("is-active", x === devicesTab));
    loadModels();
});
let models = [];
async function loadModels() {
    const p = await request("/device-models");
    models = p.models;
    document.querySelector("#device-model").innerHTML =
        models
            .map(
                (m) =>
                    '<option value="' +
            ...
            .join("") || '<option value="">Спочатку додайте модель</option>';
}
document.querySelector("#device-form").addEventListener("submit", async (e) => {
    e.preventDefault();
    await request(
        "/devices",
        "POST",
    ...
    status.textContent = "Обладнання збережено";
});
const addDeviceButton = document.createElement("button");
addDeviceButton.type = "button";
addDeviceButton.className = "text-context-menu";
addDeviceButton.textContent = "Додати в БД";
addDeviceButton.hidden = true;
document.body.append(addDeviceButton);
document.addEventListener("contextmenu", (e) => {
    const f = e.target.closest(".result-field"),
        v = f?.value.slice(f.selectionStart, f.selectionEnd) || "";
    if (!v) return;
    addDeviceButton.hidden = false;
    addDeviceButton.style.left = e.clientX + "px";
    addDeviceButton.style.top = e.clientY + 34 + "px";
    addDeviceButton.dataset.value = v;
});
addDeviceButton.addEventListener("click", () => {
    document.querySelector("#device-text").value =
        addDeviceButton.dataset.value;
...
async function showModels() {
    const p = new URLSearchParams({
        devices_type: qs("#mf-type").value,
        device_service: qs("#mf-service").value,
    });
    const d = await request("/device-models?" + p);
    qs("#models-list").innerHTML =
        "<table><tr><th>Назва</th><th>Тип</th><th>Послуга</th><th></th></tr>" +
        d.models
            .map(
                (m) =>
                    "<tr><td>" +
                    escapeHtml(m.devices_name) +
                    "</td><td>" +
                    label(m.devices_type) +
                    "</td><td>" +
                    label(m.device_service) +
                    '</td><td><button data-edit-model="' +
                    m.id +
                    '">Редагувати</button> <button data-delete-model="' +
                    m.id +
...
function openModel(m = {}) {
    const f = qs("#model-crud");
    f.elements.id.value = m.id || "";
    f.devices_name.value = m.devices_name || "";
    f.devices_type.value = m.devices_type || "modem";
    f.device_service.value = m.device_service || "internet";
    qs("#model-dialog").showModal();
}
qs("#new-model").onclick = () => openModel();
qs("#model-crud").onsubmit = async (e) => {
    e.preventDefault();
    const f = e.target,
        d = Object.fromEntries(new FormData(f));
    try {
        await request(
            d.id ? "/device-models/" + d.id : "/device-models",
            d.id ? "PUT" : "POST",
            d,
        );
        qs("#model-dialog").close();
        showModels();
        loadModels();
    } catch (x) {
        f.querySelector(".agent-message").textContent = x.message;
    }
};
qs(".cancel-dialog").onclick = () => qs("#model-dialog").close();
qs("#mf-type").onchange = showModels;
qs("#mf-service").onchange = showModels;
async function showDevices() {
    const p = new URLSearchParams({
        search: qs("#df-search").value,
        date_from: qs("#df-from").value,
        date_to: qs("#df-to").value,
        devices_type: qs("#df-type").value,
        device_service: qs("#df-service").value,
    });
    const d = await request("/devices?" + p);
    qs("#devices-list").innerHTML =
        "<table><tr><th>Дата</th><th>Текст</th><th>Модель</th><th>Тип</th><th>Послуга</th><th></th></tr>" +
        d.devices
            .map(
                (x) =>
                    "<tr><td>" +
                    formatDeviceDate(x.registered_at) +
                    "</td><td>" +
                    escapeHtml(x.recognized_text) +
                    "</td><td>" +
                    escapeHtml(x.devices_name) +
                    "</td><td>" +
                    label(x.devices_type) +
                    "</td><td>" +
                    label(x.device_service) +
                    '</td><td><button data-edit-device="' +
                    x.id +
                    '">Редагувати</button> <button data-delete-device="' +
                    x.id +
                    '">Видалити</button></td></tr>',
            )
            .join("") +
...
function editDevice(x) {
    const f = qs("#device-crud");
    f.elements.id.value = x.id;
    f.recognized_text.value = x.recognized_text;
    f.device_model_id.value = x.device_model_id;
    f.registered_at.value = x.registered_at.slice(0, 16);
    qs(".cancel-device").hidden = false;
    window.scrollTo({ top: 0, behavior: "smooth" });
}
qs("#device-crud").onsubmit = async (e) => {
    e.preventDefault();
    const f = e.target,
        d = Object.fromEntries(new FormData(f));
    await request(
        d.id ? "/devices/" + d.id : "/devices",
        d.id ? "PUT" : "POST",
        d,
    );
    f.reset();
    f.elements.id.value = "";
    qs(".cancel-device").hidden = true;
    showDevices();
};
qs(".cancel-device").onclick = () => {
    qs("#device-crud").reset();
    qs("#device-crud").elements.id.value = "";
    qs(".cancel-device").hidden = true;
};
["#df-search", "#df-from", "#df-to", "#df-type", "#df-service"].forEach(
    (s) => (qs(s).oninput = showDevices),
);
modelTabButton.onclick = () => {
    document
        .querySelectorAll(".tab-button")
        .forEach((x) => x.classList.toggle("is-active", x === modelTabButton));
    document
...
document.querySelector("#device-crud [name=registered_at]").type =
    "datetime-local";
document.querySelector("#device-crud [name=registered_at]").value = new Date()
    .toISOString()
    .slice(0, 16);
const exportDevicesButton = document.createElement("button");
exportDevicesButton.id = "export-devices";
exportDevicesButton.className = "primary-button";
exportDevicesButton.type = "button";
exportDevicesButton.textContent = "Вигрузити в Excel";
document.querySelector("#devices-list").before(exportDevicesButton);
exportDevicesButton.onclick = () => {
    const p = new URLSearchParams({
        search: qs("#df-search").value,
        date_from: qs("#df-from").value,
        date_to: qs("#df-to").value,
    ...
    window.location = "/devices-export?" + p;
};
document
    .querySelector(".tabs")
    .append(document.querySelector('[data-tab="settings"]'));
exportDevicesButton.onclick = async () => {
    ...
    const r = await fetch("/devices-export?" + p);
...
deviceFiltersElement.innerHTML =
...
const deviceQuery = () => {
    ...
    if (from && to && to < from) {
        deviceFilterError.textContent =
            "Дата «до» не може бути раніше за дату «від».";
    ...
    qs("#devices-list").innerHTML =
        ...
        data.devices
            .map(
                (device) =>
                    "<tr><td>" +
                    kyivDisplayDateTime(device.registered_at) +
                    "</td><td>" +
                    escapeHtml(device.recognized_text) +
                    "</td><td>" +
                    escapeHtml(device.devices_name) +
                    "</td><td>" +
                    label(device.devices_type) +
                    "</td><td>" +
                    label(device.device_service) +
                    '</td><td><button data-edit-device="' +
                    device.id +
                    '">Редагувати</button> <button data-delete-device="' +
                    device.id +
...
addDeviceButton.addEventListener(
    ...
    (event) => {
        ...
        document.querySelector("#device-text").value = normalizeDeviceText(
            addDeviceButton.dataset.value,
        );
        deviceTabButton.click();
        addDeviceButton.hidden = true;
...
addUnformattedDeviceButton.addEventListener(
    "click",
    (event) => {
        event.stopImmediatePropagation();
        document.querySelector("#device-text").value =
            addUnformattedDeviceButton.dataset.value;
        deviceTabButton.click();
        addUnformattedDeviceButton.hidden = true;
    },
    true,
...
const tabIcons = {
    Розпізнавання:
        '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 4h5M4 4v5M20 4h-5M20 4v5M4 20h5M4 20v-5M20 20h-5M20 20v-5M8 12h8M12 8v8"/></svg>',
    Обладнання:
        '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M8 9h8M8 13h3M8 17h8"/></svg>',
...
imageActionMenu.innerHTML =
    '<button type="button" data-image-action="rotate">Повернути на 90° вправо</button><hr><button type="button" data-image-action="delete" class="danger">Видалити фото</button><hr><button type="button" data-image-action="close" class="danger">Закрити</button>';
...
document.querySelectorAll(".image-card-menu").forEach((button) => {
    button.addEventListener(
        ...
        (event) => {
            ...
            imageActionMenu.style.left =
                Math.min(event.clientX, window.innerWidth - 230) + "px";
            imageActionMenu.style.top =
                Math.min(event.clientY, window.innerHeight - 170) + "px";
        },
        true,
    );
});
...
imageActionMenu.addEventListener("click", async (event) => {
    const action = event.target.closest("[data-image-action]")?.dataset
        .imageAction;
...
document.addEventListener(
    ...
    (event) => {
        if (
            !event.target.closest(".image-action-menu") &&
            !event.target.closest(".image-card-menu")
        ) {
            imageActionMenu.hidden = true;
        }
    },
...
document
    .querySelector("#external-homeandriy")
...
const renderAgentTable = () => {
    agentTable.innerHTML =
        "<table><tr><th>Назва</th><th>Постачальник</th><th>Модель</th><th></th></tr>" +
        agents
            .map(
                (agent) =>
                    "<tr><td>" +
                    escapeHtml(agent.name) +
                    "</td><td>" +
                    escapeHtml(providers[agent.provider]) +
                    "</td><td>" +
                    escapeHtml(agent.model) +
                    '</td><td><button data-edit-agent="' +
                    agent.id +
                    '">Редагувати</button> <button data-delete-agent="' +
                    agent.id +
                    '" class="danger-button">Видалити</button></td></tr>',
            )
            .join("") +
        "</table>";
...
qs("#agent-crud").onsubmit = async (event) => {
    ...
    try {
        const response = await request(
            data.id ? "/ai-agents/" + data.id : "/ai-agents",
            data.id ? "PUT" : "POST",
            data,
        );
        agents = data.id
            ? agents.map((agent) =>
                  agent.id === data.id ? response.agent : agent,
              )
        ...
        form.querySelector(".agent-message").textContent = error.message;
    }
};
...
unifiedOcrMenu.innerHTML =
    '<button type="button" data-ocr-action="copy"><span>⧉</span>Копіювати</button><button type="button" data-ocr-action="normal"><span>⌁</span>Додати в БД</button><button type="button" data-ocr-action="raw"><span>≡</span>Додати в БД неформатовано</button><hr><button type="button" data-ocr-action="close" class="danger"><span>×</span>Закрити</button>';
...
document.addEventListener("contextmenu", (event) => {
    ...
    unifiedOcrMenu.style.left =
        Math.min(event.clientX, window.innerWidth - 270) + "px";
    unifiedOcrMenu.style.top =
        Math.min(event.clientY, window.innerHeight - 170) + "px";
    textContextMenu.hidden = true;
    addDeviceButton.hidden = true;
    addUnformattedDeviceButton.hidden = true;
...
unifiedOcrMenu.addEventListener("click", async (event) => {
    ...
    if (action === "copy") {
        ...
        qs("#device-text").value =
            action === "normal"
                ? normalizeDeviceText(contextSelection)
                : contextSelection;
        deviceTabButton.click();
    }
...
document.addEventListener(
    ...
    (event) => {
        if (
            !event.target.closest(".context-action-menu") &&
            !event.target.closest(".image-action-menu") &&
            !event.target.closest(".image-card-menu")
        ) {
            hideAllContextActions();
...
imageActionMenu.addEventListener(
    "click",
    async (event) => {
        const action = event.target.closest("[data-image-action]")?.dataset
            .imageAction;
        ...
        try {
            ...
            const image = document.querySelector(
                '.image-card[data-image-id="' + imageId + '"] img',
            );
...
imageActionMenu.innerHTML =
    '<button type="button" data-image-action="rotate"><span>↻</span>Повернути на 90° вправо</button><button type="button" data-image-action="view"><span>◉</span>Переглянути</button><hr><button type="button" data-image-action="delete" class="danger"><span>⌫</span>Видалити фото</button><hr><button type="button" data-image-action="close" class="danger"><span>×</span>Закрити</button>';
...
window.addEventListener(
    ...
    (event) => {
        const ocrAction =
            event.target.closest?.("[data-ocr-action]")?.dataset.ocrAction;
...
folderSettings.innerHTML =
    '<h2>Папка зображень</h2><form id="image-directory-form" class="agent-form"><label>Шлях до папки<div class="setup-directory"><input name="path" readonly required><button id="choose-image-directory" class="tab-button" type="button">Вибрати папку</button></div></label><button class="primary-button">Зберегти папку</button><p class="agent-message"></p></form></section>';
settingsTabCrud.prepend(folderSettings);
const imageDirectoryForm = qs("#image-directory-form");
request("/image-directory")
    .then((payload) => (imageDirectoryForm.path.value = payload.path))
    .catch(
        (error) =>
            (imageDirectoryForm.querySelector(".agent-message").textContent =
                error.message),
    );
qs("#choose-image-directory").onclick = async () => {
    const message = imageDirectoryForm.querySelector(".agent-message");
    try {
        message.textContent = "Оберіть папку у вікні Windows…";
        const payload = await request("/image-directory/choose", "POST");
        if (payload.path) {
            imageDirectoryForm.path.value = payload.path;
            message.textContent = "Папку вибрано. Натисніть «Зберегти папку».";
        } else {
            message.textContent = "Вибір папки скасовано.";
        }
    } catch (error) {
        message.textContent = error.message;
    }
...
imageDirectoryForm.onsubmit = async (event) => {
    ...
    try {
        await request(
            "/image-directory",
            "PUT",
            Object.fromEntries(new FormData(imageDirectoryForm)),
        );
        window.location.reload();
    } catch (error) {
        message.textContent = error.message;
    ...
    list.innerHTML =
        ...
        data.devices
            .map(
                (d) =>
                    "<tr><td>" +
                    kyivDisplayDateTime(d.registered_at) +
                    "</td><td>" +
                    escapeHtml(d.recognized_text) +
                    "</td><td>" +
                    escapeHtml(d.devices_name) +
                    "</td><td>" +
                    label(d.devices_type) +
                    "</td><td>" +
                    label(d.device_service) +
                    '</td><td><button data-edit-device="' +
                    d.id +
                    '">Редагувати</button> <button data-delete-device="' +
                    d.id +
        ...
        (data.pagination
            ? '<div class="pagination"><button data-page="prev" ' +
              ...
              ">‹</button><span>Сторінка " +
              data.pagination.page +
              " з " +
              data.pagination.pages +
              " · " +
              data.pagination.total +
              ' записів</span><button data-page="next" ' +
...
["#df-search", "#df-from", "#df-to", "#df-type", "#df-service"].forEach((s) =>
    qs(s).addEventListener("change", () => {
        devicesPage = 1;
    }),
);
...
showModels = async () => {
    ...
    qs("#models-list").innerHTML =
        ...
        "</table>" +
        (data.pagination
            ? '<div class="pagination"><button data-model-page="prev" ' +
              (data.pagination.page === 1 ? "disabled" : "") +
              ">‹</button><span>Сторінка " +
              data.pagination.page +
              " з " +
              ...
              " · " +
              data.pagination.total +
              ' моделей</span><button data-model-page="next" ' +
              (data.pagination.page === data.pagination.pages
                  ? "disabled"
                  : "") +
              ">›</button></div>"
    ...
    qs('[data-model-page="prev"]')?.addEventListener("click", () => {
        modelsPage--;
        showModels();
    });
    qs('[data-model-page="next"]')?.addEventListener("click", () => {
        modelsPage++;
        showModels();
    });
};
["#mf-type", "#mf-service"].forEach((s) =>
    qs(s).addEventListener("change", () => {
        modelsPage = 1;
    }),
);
document
    .querySelector("#open-image-directory")
    ?.addEventListener("click", async () => {
        try {
            await request("/image-directory/open", "POST");
        } catch (error) {
            status.textContent = error.message;
        }
    });
...
if (firstRunSetup) {
    const setupAccepted = document.querySelector("#setup-accepted"),
        setupDirectory = document.querySelector("#setup-image-directory"),
        setupChoose = document.querySelector("#choose-setup-directory"),
        setupComplete = document.querySelector("#complete-setup"),
        setupMessage = document.querySelector("#setup-message");
    const updateSetupState = () => {
        setupComplete.disabled = !(
            setupAccepted.checked && setupDirectory.value.trim()
        );
    };
    setupAccepted.addEventListener("change", updateSetupState);
    setupChoose.addEventListener("click", async () => {
        setupChoose.disabled = true;
        setupMessage.textContent = "Відкриваємо вибір папки…";
        try {
            const result = await request("/image-directory/choose", "POST");
            if (result.path) setupDirectory.value = result.path;
            setupMessage.textContent = result.path
                ? "Папку вибрано."
                : "Вибір скасовано.";
            updateSetupState();
        } catch (error) {
    ...
    setupComplete.addEventListener("click", async () => {
        setupComplete.disabled = true;
        setupMessage.textContent = "Зберігаємо налаштування…";
        try {
            await request("/setup", "POST", {
                accepted: setupAccepted.checked,
                path: setupDirectory.value,
            });
            if (document.body.dataset.environment === "local") {
                firstRunSetup.remove();
            } else {
                window.location.reload();
            }
        } catch (error) {
...
showDevices = async () => {
    ...
    const list = qs("#devices-list");
    list.innerHTML =
        "<table><tr><th>Дата</th><th>Договір</th><th>Операція</th><th>Текст</th><th>Модель</th><th>Тип</th><th>Послуга</th><th></th></tr>" +
        data.devices
            .map(
                (device) =>
                    "<tr><td>" +
                    kyivDisplayDateTime(device.registered_at) +
                    "</td><td>" +
                    escapeHtml(device.contract_number || "—") +
                    "</td><td>" +
                    operationLabel(device.operation_type) +
                    "</td><td>" +
                    escapeHtml(device.recognized_text) +
                    "</td><td>" +
                    escapeHtml(device.devices_name) +
                    ...
                    (device.source_image_path
                        ? '<button data-open-source-image="' +
                          device.id +
                          '">Відкрити файл</button> '
                        : "") +
                    '<button data-edit-device="' +
                    device.id +
                    '">Редагувати</button> <button data-delete-device="' +
                    device.id +
                    '">Видалити</button></td></tr>',
    ...
    data.devices.forEach((device) => {
        qs('[data-open-source-image="' + device.id + '"]')?.addEventListener(
            "click",
            async () => {
                try {
                    await request(
                        "/devices/" + device.id + "/source-image/open",
                        "POST",
                    );
                } catch (error) {
                    alert(error.message);
...
unifiedOcrMenu.addEventListener("click", (event) => {
    ...
    if (action === "normal" || action === "raw") {
        qs("#device-crud").elements.source_image_id.value =
            selectedImageId || "";
    }
});
...
popularModelsContainer.innerHTML =
    '<span class="popular-models-title">Популярні моделі</span><div class="popular-model-buttons"></div>';
qs("#device-model")
    .closest("label")
    .insertAdjacentElement("afterend", popularModelsContainer);

const showPopularModels = async () => {
    const data = await request("/device-models/popular");
    const buttons = popularModelsContainer.querySelector(
        ".popular-model-buttons",
    );
    buttons.innerHTML = data.models
        .map(
            (model) =>
                '<button type="button" data-popular-model-id="' +
                model.id +
                '" title="Обрати ' +
                escapeHtml(model.devices_name) +
                '">' +
                escapeHtml(model.devices_name) +
                "</button>",
        )
...
document
    .querySelector("#about-open-site")
    ?.addEventListener("click", openDeveloperWebsite);
document
    .querySelector("#about-external-homeandriy")
    ?.addEventListener("click", (event) => {
        event.preventDefault();
        openDeveloperWebsite();
    });
const registerAboutNativeEvent = () =>
    window.Native?.on?.("App\\Events\\ShowAboutDialog", showAboutDialog);
window.addEventListener("native:init", registerAboutNativeEvent, {
    once: true,
});
...
const registerStartupLogNativeEvent = () =>
    window.Native?.on?.("App\\Events\\OpenStartupLog", openStartupLog);
window.addEventListener("native:init", registerStartupLogNativeEvent, {
    once: true,
});
...
const startManualUpdateCheck = () => {
    ...
    manualUpdateCheckTimeout = window.setTimeout(() => {
        finishManualUpdateCheck(
            "Перевірка оновлень триває надто довго. Перевірте підключення до інтернету та спробуйте ще раз.",
        );
    }, 30000);
};
...
const registerUpdateNativeEvents = () => {
    ...
    window.Native?.on?.(
        ...
        () =>
            finishManualUpdateCheck("У вас уже встановлена актуальна версія."),
    );
    window.Native?.on?.(
        ...
        (payload) =>
            finishManualUpdateCheck(
                "Доступне оновлення" +
                    ...
                    ". Завантажуємо та встановлюємо його…",
            ),
    );
    window.Native?.on?.(
...
window.addEventListener("native:init", registerUpdateNativeEvents, {
    once: true,
});
...
document
    .querySelector(".tabs")
    .insertBefore(statisticsTabButton, settingsTabButton);
...
for (let offset = 0; offset < 12; offset++) {
    const month = new Date(
        Date.UTC(
            ...
            statisticsNow.getMonth() - offset,
            1,
        ),
    );
    const value = month.toISOString().slice(0, 7);
    statisticsMonth.insertAdjacentHTML(
        "beforeend",
        '<option value="' +
            value +
            '">' +
            statisticsMonthFormatter.format(month) +
            "</option>",
    );
}
...
const statisticsPercent = (value, maximum) =>
    Math.max(4, Math.round((value / Math.max(maximum, 1)) * 100));

const statisticsOperationChart = (rows) => {
    if (!rows.length)
        return statisticsEmpty("Ще немає записів для побудови графіка.");

    const maximum = Math.max(
        ...rows.flatMap((row) => [row.receipt, row.issue]),
        1,
    );

    return (
        '<div class="statistics-chart-scroll"><div class="statistics-column-chart statistics-operation-chart">' +
        rows
            .map(
                (row) =>
                    '<div class="statistics-column" title="' +
                    escapeHtml(
                        row.label +
                            ": прийом " +
                            row.receipt +
                    ...
                    '"><div class="statistics-bars"><span class="statistics-bar receipt" style="height:' +
                    statisticsPercent(row.receipt, maximum) +
                    '%"></span><span class="statistics-bar issue" style="height:' +
                    statisticsPercent(row.issue, maximum) +
                    '%"></span></div><strong>' +
                    row.total +
                    "</strong><small>" +
...
const statisticsCountChart = (rows, labelKey) => {
    ...
    return (
        '<div class="statistics-chart-scroll"><div class="statistics-column-chart">' +
        rows
            .map(
                (row) =>
                    '<div class="statistics-column" title="' +
                    escapeHtml(row[labelKey] + ": " + row.total) +
                    '"><div class="statistics-bars"><span class="statistics-bar total" style="height:' +
                    statisticsPercent(row.total, maximum) +
                    '%"></span></div><strong>' +
                    row.total +
                    "</strong><small>" +
...
const renderStatistics = (data) => {
    ...
    statisticsContent.innerHTML =
        '<section class="statistics-card"><div class="statistics-card-heading"><div><h3>Прийом та видача</h3><p>Кількість операцій ' +
        (data.group_by === "month" ? "по місяцях" : "по днях") +
        '.</p></div><div class="statistics-legend"><span><i class="receipt"></i>Прийом</span><span><i class="issue"></i>Видача</span></div></div>' +
        statisticsOperationChart(data.operations) +
        '</section><section class="statistics-card"><div class="statistics-card-heading"><div><h3>Послуги</h3><p>Інтернет і телебачення.</p></div></div>' +
        statisticsCountChart(data.services, "label") +
        '</section><section class="statistics-card"><div class="statistics-card-heading"><div><h3>Обладнання</h3><p>Кількість заявок за назвою обладнання.</p></div></div>' +
        statisticsCountChart(data.models, "name") +
        "</section>";
};

const loadStatistics = async () => {
    statisticsContent.innerHTML =
        '<p class="empty-state">Завантажуємо статистику…</p>';

    try {
        const parameters = new URLSearchParams({ group_by: statisticsGroupBy });
        if (statisticsGroupBy === "month")
            parameters.set("month", statisticsMonth.value);
...
statisticsTabButton.addEventListener("click", () => {
    document
        .querySelectorAll(".tabs > .tab-button")
        .forEach((button) =>
    ...
    document
        .querySelectorAll(".tab-content")
        .forEach((tab) =>
            tab.classList.toggle("is-active", tab === statisticsTab),
        );
    statisticsTab
        .querySelectorAll("[data-statistics-group]")
        .forEach((button) =>
            button.classList.toggle(
                "is-active",
                button.dataset.statisticsGroup === statisticsGroupBy,
            ),
        );
...
if (appVersionFooter)
    document.querySelector(".app-shell")?.append(appVersionFooter);
...
const renderImageCatalog = (images) => {
    if (!images.length) {
        imageCatalogElement.innerHTML =
            '<p class="empty-state">У папці немає зображень. Додайте JPG, PNG, WEBP, TIFF або BMP до вказаної папки.</p>';
        return;
    }
    ...
    imageCatalogElement.innerHTML = Object.values(groups)
        .map(
            (imagesForDay) =>
                '<section class="image-day"><div class="image-day-divider"><span>' +
                escapeHtml(imagesForDay[0].uploaded_label) +
                '</span></div><div class="image-grid">' +
                imagesForDay
                    .map(
                        (image) =>
                            '<article class="image-card" data-image-id="' +
                            image.id +
                            '" data-image-name="' +
                            escapeHtml(image.name) +
                            '"><button class="image-preview" type="button"><img src="/images/' +
                            image.id +
                            '" alt="' +
                            escapeHtml(image.name) +
                            '" loading="lazy"></button><div class="image-card-footer"><span title="' +
                            escapeHtml(image.name) +
                            '">' +
                            escapeHtml(image.name) +
                            '</span><button class="image-card-menu" type="button" aria-label="Меню фото">⋮</button></div></article>',
                    )
                    .join("") +
                "</div></section>",
        )
...
const renderImagePagination = (data) => {
    imagePaginationElement.innerHTML =
        '<button class="tab-button" type="button" data-image-page="' +
        (data.page - 1) +
        '" ' +
        (data.page === 1 ? "disabled" : "") +
        ">Попередня</button><span>Показано " +
        data.images.length +
        " з " +
        data.total +
        '</span><button class="tab-button" type="button" data-image-page="' +
        (data.page + 1) +
        '" ' +
...
const loadImageCatalog = async (page) => {
    imageCatalogElement.innerHTML =
        '<p class="empty-state">Завантажуємо зображення…</p>';

    try {
        const data = await request(
            "/image-catalog?page=" + page + "&per_page=48",
        );
...
imagePaginationElement?.addEventListener("click", (event) => {
    const page = Number(
        event.target.closest("[data-image-page]")?.dataset.imagePage,
    );
    if (Number.isInteger(page) && page > 0 && page !== imageCatalogPage)
        loadImageCatalog(page);
});
