...
final class DeviceController extends Controller{
 ...
 public function export(Request $r):\Symfony\Component\HttpFoundation\StreamedResponse{$items=$this->query($r)->get();return response()->streamDownload(function()use($items){echo "\xEF\xBB\xBF"; $out=fopen('php://output','w');fputcsv($out,['Дата','Текст','Модель','Тип','Послуга'],';');foreach($items as$d)fputcsv($out,[$d->registered_at->timezone('Europe/Kyiv')->format('d.m.Y H:i'),$d->recognized_text,$d->devices_name,$d->devices_type,$d->device_service],';');fclose($out);},'equipment.csv',['Content-Type'=>'text/csv; charset=UTF-8']);}
